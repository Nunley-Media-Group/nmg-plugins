...
| When specifying features that consume configuration or context documents at multiple processing stages, the spec often references "context documents" generically without enumerating which documents are read at which stage | Include ACs that enumerate which context sources (config files, steering docs, state files) are read at each processing step, rather than referencing them as a generic collection | .claude/specs/bug-fix-draft-issue-steering-docs/ |
