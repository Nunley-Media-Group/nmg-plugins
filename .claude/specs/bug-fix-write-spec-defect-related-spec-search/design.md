...
|------|--------|-----------|
| `plugins/nmg-sdlc/skills/write-spec/SKILL.md` | Replace the passive defect instruction in step 7 with an active search step: extract keywords from the issue, glob+grep `.claude/specs/*/requirements.md`, populate Related Spec with match or N/A | Gives the agent a concrete discovery mechanism instead of relying on passive intuition |
