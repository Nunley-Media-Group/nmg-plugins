...
The three output documents — `product.md`, `tech.md`, and `structure.md` — are written to `.claude/steering/` and provide the foundation that `/draft-issue`, `/write-spec`, `/write-code`, and `/verify-code` all reference for project-specific context. The skill also creates an empty `.claude/specs/` directory for future spec storage.
