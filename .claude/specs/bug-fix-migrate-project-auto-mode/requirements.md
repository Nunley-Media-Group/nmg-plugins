# Defect Report: migrate-project Respects auto-mode Despite Spec Excluding It
...
| | Description |
|---|-------------|
| **Expected** | `/migrate-project` always presents proposed changes for interactive review via `AskUserQuestion` before modifying any files, regardless of whether `.claude/auto-mode` exists |
...
**When** any other SDLC skill with auto-mode support runs (e.g., `/write-spec`)
...
|----|-------------|----------|
| FR1 | The `/migrate-project` skill MUST always use `AskUserQuestion` for the review gate, regardless of `.claude/auto-mode` | Must |
...
- Adding auto-mode support to `/migrate-project` (the original spec explicitly excludes it)
