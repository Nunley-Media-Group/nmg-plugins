...
This feature adds a new `/run-retro` skill to the nmg-sdlc plugin and a small integration point in `/write-spec`. The skill scans all defect specs in `.claude/specs/`, identifies those with `Related Spec` links back to feature specs, analyzes the gap between what the feature spec covered and what the defect revealed, classifies findings into three pattern types, and writes a structured `.claude/steering/retrospective.md` steering document. The `/write-spec` skill then reads this document during Phase 1 alongside the existing steering docs.
...
│  Skills (existing)                                          │
│  ┌─────────────────┐                                        │
│  │ /write-spec  │──reads──▶ .claude/steering/*           │
